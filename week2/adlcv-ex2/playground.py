...
def image_to_patches(image, patch_size, image_grid=True):
    ...
    if image_grid:
        ...
        """
        patches = rearrange(image, 'B C (num_patches_h patch_h) (num_patches_w patch_w) -> B C num_patches_h patch_h num_patches_w patch_w', patch_h=patch_h, patch_w=patch_w)
        ...
        """
        patches = rearrange(patches, 'B C num_patches_h patch_h num_patches_w patch_w -> B (num_patches_h num_patches_w) C patch_h patch_w', patch_h=patch_h, patch_w=patch_w)
    else:
        """
